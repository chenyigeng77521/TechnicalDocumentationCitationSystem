...
export class DocumentConverter {
     ...
     * 转换文件为 Markdown
     */
    async convert(filePath, originalFileName) {
    ...
    async convertPdf(filePath) {
        const data = await pdfParse(Buffer.from(filePath));
     ...
     * 转换 Markdown 文件
     */
    async convertMd(filePath) {
        const content = fs.readFileSync(filePath, 'utf-8');
        const lines = content.split('\n');
        const lineMappings = lines
            .map((line, idx) => ({
            mdLine: idx + 1,
            originalLine: idx + 1,
            content: line,
     ...
     * 构建行映射
     */
    buildLineMappings(mdContent, prefix = '') {
