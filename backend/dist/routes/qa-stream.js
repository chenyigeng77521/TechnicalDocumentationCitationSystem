...
router.post('/ask-stream', async (req, res) => {
    try {
        const { question, topK = 5, strictMode = true } = req.body;
        ...
        const chunkRecords = db.searchChunks(query).slice(0, topK);
        // 转换为 RetrievedChunk 类型
        const chunks = chunkRecords.map((chunk, idx) => {
            ...
            return {
                ...
                fileName: file?.original_name || '未知文件'
            };
        });
        if (chunks.length === 0) {
        ...
        // 构建提示词
        const contextText = chunks.map((chunk, idx) => `【文档 ${idx + 1}】${chunk.content.substring(0, 200)}...`).join('\n\n---\n\n');
...
请开始回答：`;
        ...
        // 4. 发送引用信息
        const citationInfo = chunkRecords.map((c, idx) => {
...
//# sourceMappingURL=qa-stream.js.map