...
export async function classifyQuestion(question) {
    ...
    try {
        ...
        const response = await axios.post(`${config.firstlayer.url}/api/classify`, { question }, {
...
export async function getQuestionTypes() {
    ...
    try {
        const response = await axios.get(`${config.firstlayer.url}/api/classify/types`, { timeout: config.firstlayer.timeout });
 ...
 * @returns 检索策略描述
 */
export function getSearchStrategy(category) {
    const strategies = {
...
export function isValidCategory(category) {
    const validCategories = ['FACT', 'PROC', 'EXPL', 'COMP', 'META', 'UNKNOWN'];
    return validCategories.includes(category);
}
//# sourceMappingURL=firstlayer-client.js.map