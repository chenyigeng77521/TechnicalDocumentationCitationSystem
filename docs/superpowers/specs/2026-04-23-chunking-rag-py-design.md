...
前端 [page.tsx:35,54,144,191](../../../frontend/app/page.tsx) 四处 `fetch()` 调用里**直接写死字符串** `http://localhost:3002`——**不经过** `NEXT_PUBLIC_API_URL` 环境变量（该 env 在 frontend 代码里并未被读取）。因此端口必须是 3002，不可通过环境变量覆盖。
...
- 路由通过 `Depends(get_embedder)` / `Depends(get_reranker)` 拿单例——模型对象本身线程安全（FlagEmbedding 用 torch，read-only 推理）
...
def get_db(settings: Settings = Depends(get_settings)):
    ...
    try:
        ...
        conn.close()
```
- WAL 模式：多个读请求并发无阻塞；写请求走 `BEGIN IMMEDIATE` 拿 RESERVED 锁，串行化靠 SQLite 文件锁（非 Python GIL、非 threading.Lock）
- 写事务显式写：`with db.conn:` 块或 `db.conn.execute("BEGIN IMMEDIATE")` 开场
...
lsof -ti:3002 2>/dev/null | xargs -r kill -9 2>/dev/null || true
...
multipart → FastAPI UploadFile
  ...
  │    [database] 单事务：INSERT chunks × N (vector=JSON.dumps(v))
  │    [database] UPDATE files SET status='completed' WHERE id=?
  │
...
| `test_cors.py` | 2 | OPTIONS preflight（`Origin`/`Access-Control-Request-Method` 头）返回 204 + 正确 `Access-Control-Allow-*`；未配置 origin 拒绝 |
...
- **缓解**：README 写 `lsof -ti:3002 2>/dev/null | xargs -r kill -9 2>/dev/null || true`（`-r` 防止空输入报错）作为启动前置步骤
- **剩余风险**：演示现场端口抢占看现场纪律
...
python -c "from FlagEmbedding import BGEM3FlagModel; m = BGEM3FlagModel('BAAI/bge-m3', use_fp16=True); print(m.encode(['hello']).shape)"
