...
async def filter_question(request: FilterRequest):
    ...
    # 📝 调用接口前日志
    import logging
    logger = logging.getLogger("question_filter")
    logger.info(f"📥 [filter_question] 收到请求 - question: {request.question[:50]}... history_len: {len(request.conversation_history or [])}")
    ...
    **示例**:
    ...
    try:
        ...
        import logging
        logger = logging.getLogger("question_filter")
        logger.error(f"❌ [filter_question] 接口调用失败 - error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"过滤失败：{str(e)}")
    
    # 📝 调用接口后日志（成功）
    import logging
    logger = logging.getLogger("question_filter")
    logger.info(f"✅ [filter_question] 接口调用成功 - category: {category}, confidence: {result.get('confidence')}")
