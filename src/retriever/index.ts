...
import { Chunk, Citation, SearchResult, SearchRequest, SearchResponse } from '../types.js';
...
export class Retriever {
  ...
  async search(request: SearchRequest): Promise<SearchResponse> {
    ...
    if (this.openai) {
      try {
        ...
        // 获取所有带向量的文档块
        const allChunks = this.db.getAllFiles().flatMap(file => 
          this.db.getFileChunks(file.id).filter(c => c.vector && c.vector.length > 0)
        );
    ...
    // 构建搜索结果
    const results: SearchResult[] = candidates.slice(0, topK).map(chunk => ({
   ...
   * 为文档块计算向量并存储
   */
  async indexChunks(fileId: string): Promise<void> {
    if (!this.openai) {
