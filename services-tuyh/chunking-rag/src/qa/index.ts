...
import { QARequest, QAResponse, Citation, SearchResult } from '../types.js';
...
export class QAAgent {
  ...
  private strictModeDefault: boolean;
   ...
   * 回答问题
   */
  async answer(request: QARequest): Promise<QAResponse> {
   ...
   * 无据拒答响应
   */
  private noEvidenceResponse(question: string): QAResponse {
    return {
   ...
   * 简单响应（无 LLM）
   */
  private simpleResponse(searchResults: { results: SearchResult[] }): QAResponse {
   ...
   * 构建提示词
   */
  private buildPrompt(question: string, results: SearchResult[], strictMode: boolean): string {
...
`;
   ...
   * 解析引用
   */
  private parseCitations(llmResponse: string, results: SearchResult[]): {
    answer: string;
    citations: Citation[];
  } {
    ...
    while ((match = citationRegex.exec(llmResponse)) !== null) {
      ...
      const matchingResult = results.find(r => 
        r.citations.some(c => 
          c.documentPath === docPath && c.paragraph === paragraph
        )
      );
   ...
   * 计算置信度
   */
  private calculateConfidence(parsed: { answer: string; citations: Citation[] }): number {
    if (parsed.citations.length === 0) return 0;
   ...
   * 获取所有可用文件列表
   */
  getAvailableFiles(): Array<{ id: string; name: string; format: string }> {
